...
BOOST_AUTO_TEST_CASE(grouptoken_basicfunctions)
{
    ...
    // Now test transaction balances
    {
        ...
        COutPoint p2sh1 = AddUtxo(p2sh(sid), 1, coins);
        {
            ...
            CScript opretScript;
            uint64_t grpNonce = 0;
            CGroupTokenID grpID = findGroupId(putxo, opretScript, GroupTokenIdFlags::NONE, grpNonce);
            ...
            grpID = findGroupId(putxo, opretScript, GroupTokenIdFlags::NONE, grpNonce);
            ...
            grpID = findGroupId(putxo, CScript(), GroupTokenIdFlags::NONE, grpNonce);
...
CGroupTokenID findGroupId(const COutPoint &input, GroupTokenIdFlags flags, uint64_t &nonce)
{
    CGroupTokenID ret;
    do
    {
...
BOOST_FIXTURE_TEST_CASE(grouptoken_blockchain, TestChain100Setup)
{
    ...
    // Create group
    uint64_t nonce;
    CGroupTokenID gid = findGroupId(COutPoint(coinbaseTxns[0].GetHash(), 0), GroupTokenIdFlags::NONE, nonce);
    ...
    CGroupTokenID gid1 = findGroupId(COutPoint(coinbaseTxns[3].GetHash(), 0), GroupTokenIdFlags::NONE, nonce);
