 ...
 */
static void CheckPassReverse(const uint32_t flags, const ReverseTestCase &reverse_case)
{
    CheckPassIfEnabled(flags, {reverse_case.item}, CScript() << OP_REVERSEBYTES, {reverse_case.reversed_item});
    CheckPassIfEnabled(
        flags, {reverse_case.item}, CScript() << OP_DUP << OP_REVERSEBYTES << OP_REVERSEBYTES << OP_EQUALVERIFY, {});
}
...
BOOST_AUTO_TEST_CASE(op_reversebytes_tests)
{
    ...
    std::vector<ReverseTestCase> test_cases({
        {{}, {}}, {{99}, {99}}, {{0xde, 0xad}, {0xad, 0xde}}, {{0xde, 0xad, 0xa1}, {0xa1, 0xad, 0xde}},
        {{0xde, 0xad, 0xbe, 0xef}, {0xef, 0xbe, 0xad, 0xde}}, {{0x12, 0x34, 0x56}, {0x56, 0x34, 0x12}},
    });
    ...
    for (int i = 0; i < 4096; i++)
    {
        ...
        {
            // Verify palindrome.
            CheckPassIfEnabled(flags, {palindrome}, CScript() << OP_DUP << OP_REVERSEBYTES << OP_EQUALVERIFY, {});
        }
