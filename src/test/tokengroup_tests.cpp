...
public:
    ...
    InputDataCopy(const CTransaction &_prevTx, int _prevout, const CKey &_key, bool _p2pkh = true)
        : InputData(prevtxStorage, _prevout, _key, _p2pkh), prevtxStorage(_prevTx)
    {
    }
};
...
// must wrap the class so I can access the protected members and do stuff that would normally be illegal like
// make an ID that's too long
class DebugCTokenGroupID : public CTokenGroupID
{
public:
    DebugCTokenGroupID(const std::vector<unsigned char> &id) : CTokenGroupID(id) {}
    void extendBy(int bytes) { data.resize(data.size() + bytes); }
};

// passed size is the total size of the subgroup
CTokenGroupID MakeSubgroup(CTokenGroupID g, int xtra, unsigned int size = 0)
{
    int gsize = g.bytes().size();
    if (size == 0)
        size = gsize + 1; // 0 means default which is 1 byte bigger

    // CTokenGroupID has size checks.  This code defeats them so we can test consensus size checks.
    int hackExtraPad = 0;
    if (size > MAX_SCRIPT_ELEMENT_SIZE)
    {
        hackExtraPad = size - MAX_SCRIPT_ELEMENT_SIZE;
        size = MAX_SCRIPT_ELEMENT_SIZE;
    }
    std::vector<unsigned char> sgbytes(size);
    ...
    DebugCTokenGroupID ret(sgbytes);
    if (hackExtraPad)
        ret.extendBy(hackExtraPad);
    return ret;
}
...
BOOST_AUTO_TEST_CASE(tokengroup_basicfunctions)
{
    ...
    bool opgEnforcing = enforceOpGroupStartHeight;
    enforceOpGroupStartHeight = true;
    ...
    enforceOpGroupStartHeight = opgEnforcing;
}
