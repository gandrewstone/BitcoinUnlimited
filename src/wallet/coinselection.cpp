...
bool extend(const CAmount targetValue,
    ...
    unsigned int changeLen)
{
    ...
    if (large == aEnd)
    {
        ...
        {
            LOG(SELECTCOINS, "Every available UTXO does not add up to %llu\n", targetValue);
            return TxoGroup(0, TxoItVec());
        }
    }
    ...
    while (!done)
    {
        ...
        extend(targetValue + fee, available, grp, solutions, 0);
        ...
        if (!done) // Now grab a random Txo and search for a solution near it
        {
            ...
            if (rit != aEnd)
            {
                ...
                extend(targetValue, available, grp2, solutions, 0); // And attempt to extend it for solutions.
