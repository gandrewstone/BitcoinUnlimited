...
int MAX_FEE_PERCENT_OF_VALUE = 25;
...

    {
        LOCK(cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoinsBU(nValueToSelect, selectionFeeRate, P2PKH_LEN, setCoins, nValueIn, coinControl))
                {
                ...
                do
                {
                    ...
                    *static_cast<CTransaction *>(&wtxNew) = CTransaction(txNew);
                    ...
                    if ((MAX_FEE_PERCENT_OF_VALUE >= 0) && (nFeeNeeded > nValue * MAX_FEE_PERCENT_OF_VALUE / 100))
                    {
                        strFailReason =
                            strprintf(_("Fee: %ld is larger than configured percent: %d of transaction value: %ld"),
                                nFeeNeeded, MAX_FEE_PERCENT_OF_VALUE, nValue);
                        return false;
                    }
