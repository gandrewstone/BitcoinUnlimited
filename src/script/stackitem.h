...
public:
    ...
    StackItem(const std::vector<unsigned char>::reverse_iterator begin, const std::vector<unsigned char>::reverse_iterator end):
    type(StackElementType::VCH), vch(begin, end)
    {
    }
    StackItem(const std::vector<unsigned char>::const_reverse_iterator begin, const std::vector<unsigned char>::const_reverse_iterator end):
    type(StackElementType::VCH), vch(begin, end)
    {
    }
    */
...
StackItem(VchStackType, unsigned int i):type(StackElementType::VCH),vch(i) {}
    ...
    // As std::vector, passing in a number allocates a vch of that length
StackItem(VchStackType, unsigned int count, const unsigned char value):type(StackElementType::VCH),vch(count,value) {}
    ...
    StackItem(const std::initializer_list<unsigned char>& ini):type(StackElementType::VCH), vch(ini.begin(), ini.end())
    {}
    ...
    {
        if (type != t) throw BadOpOnType("Invalid operation on stack type");
    }

    // No BigNum is "empty"
    bool empty() const
    {
        return (isVch() && vch.empty());
    }
